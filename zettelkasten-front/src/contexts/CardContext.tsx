...
const PartialCardContext = createContext<PartialCardContextType | undefined>(undefined);

export const PartialCardProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    const [partialCards, setPartialCards] = useState<PartialCard[]>([]);
    const [refreshPartialCards, setRefreshPartialCards] = useState<boolean>(false);
    ...
    return (
        <PartialCardContext.Provider value={{ partialCards, refreshPartialCards, setRefreshPartialCards, getPartialCards }}>
            {children}
...
export const usePartialCardContext = () => {
    ...
    if (context === undefined) {
        throw new Error('usePartialCardContext must be used wtihin a PartialCardProvider');
    }
    return context;
}